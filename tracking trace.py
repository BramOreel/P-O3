...
B = 112.6               #invullen           #Distance between the cameras [cm]
...
def planeFit(coordinaten):
    ...
    plot_3d(
        ...
        plane.plotter(alpha=0.4, lims_x=(-10, 10), lims_y=(-10, 10)),
    )
    plt.xlabel("x")
    plt.ylabel("y")
    return plane.normal
...
def bepaalHoek(x, y, z):
    ...
    normaalUiteinden = planeFit(coorOmOorsprong)
    scalairProduct = np.dot(normaalMiddelpunten,normaalUiteinden)
    plt.show()
    return np.arccos(abs(scalairProduct))/np.pi*180
